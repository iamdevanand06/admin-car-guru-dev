<?php $page = 'edit-role'; ?>
@extends('layouts.app', ['activePage' => 'table', 'title' => 'Create Vehicle Variant - Admin Panel - CarGuru', 'navName' => 'Table List', 'activeButton' => 'laravel'])
@section('content')
    <div class="page-wrapper">
        <div class="content">
            <div class="page-header">
                <div class="add-item d-flex">
                    <div class="page-title">
                        <h4 class="fw-bold">Create Variant</h4>
                        <h6>Create new Variant</h6>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('variants.store') }}" class="add-role-form">
                @csrf
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="brand_id" id="brand_id" class="form-control">
                                                    ...
                                                    @foreach($makes as $make)
                                                        <option value="{{ $make->id }}">{{ $make->brand_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                                ...
                                                <select name="model_id" id="model_id" class="form-control">
                                                    ...
                                                    @foreach($makes as $make)
                                                        @foreach($models->where('brand_id', $make->id) as $model)
                                                            <option value="{{ $model->id }}" data-brand="{{ $make->id }}">
                                                                {{ $model->model_name }}
                                                            </option>
                                                        @endforeach
                                                    @endforeach
